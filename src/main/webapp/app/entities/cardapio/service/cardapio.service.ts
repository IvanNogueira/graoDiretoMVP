...
type RestOf<T extends ICardapio | NewCardapio> = Omit<T, 'dataCriacao'> & {
...
export class CardapioService {
  ...
  protected convertDateFromClient<T extends ICardapio | NewCardapio | PartialUpdateCardapio>(cardapio: T): RestOf<T> {
    return {
      ...cardapio,
      dataCriacao: cardapio.dataCriacao?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restCardapio: RestCardapio): ICardapio {
    return {
      ...restCardapio,
      dataCriacao: restCardapio.dataCriacao ? dayjs(restCardapio.dataCriacao) : undefined,
    };
  }
